...
Thanks for helping improve TMHI Control Center.
...
```bash
docker build -t tmhi-control-center:test .
```

The active GitHub Actions workflow also builds pull requests and publishes the
Docker Hub image from `main` after `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN`
are configured in the GitHub repository.

## Pull requests

- Keep the gateway automation safe by default.
